...
within a new `plugins` configuration block, specifying the OCI image URL, the
...
OpenBao's configuration file using a new `plugins` block. For each plugin, you
will provide:
...
**Example `plugins` configuration block:**

```hcl
plugins {
  "my-secret-plugin" {
    url         = "oci://myregistry.com/openbao/plugins/my-secret-plugin:1.0.0@sha256:abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890"
    binary_name = "my-secret-plugin"
    version     = "1.0.0"
    sha256sum   = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
  }
  "another-auth-plugin" {
    url         = "oci://anotherregistry.io/openbao/plugins/auth-plugin:2.1.0@sha256:fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210"
...
1.  **Configuration Parsing:** OpenBao's configuration loader will be extended
    to parse a new top-level `plugins` block. This block will be a map where
    keys are logical plugin names (e.g., `"my-secret-plugin"`) and values are
    objects containing `url` (string), `binary_name` (string), and `sha256sum`
    (string).
...
3.  **Startup Plugin Loading Logic (Per Plugin):**
      ...
      * **OCI Image Pull:** OpenBao will use an OCI client library (e.g.,
        ...
        the `url`. This URL is expected to support standard OCI registry
...
This proposal significantly improves OpenBao's plugin security posture:
  ...
  * **Immutable Image Digests:** Encouraging the use of OCI image digests in the
    `url` (e.g., `oci://...@sha256:digest`) provides strong content
