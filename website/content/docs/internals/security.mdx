...
To make an analogy, a bank puts security deposit boxes inside of a openbao. Each
security deposit box has a key, while the openbao door has both a combination and
a key. The openbao is encased in steel and concrete so that the door is the only
